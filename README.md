...
MEASpikeR can be downloaded at https://gitlab.com/icm-institute/dac/biostats/MEASpikeR. It can be installed as a regular R package. 
...
Data are generated automatically from recordings. **These files consist of “frequency” and “spike” CSV files that need to be stored in the same folder called “data” to be used by MEASpikeR.** This is a view of the `\data` folder without the guideline (we will explain how to create it in the next paragraph):

![](img/data_without_guideline.png)

## Step 2: Create your guideline file

The guideline is required to perform customized and multiple analyses of a single recording file. The guideline is an excel file which contains the name of the recording file and with the setting parameters to be chosen by the user. These parameters include, among others, the time window for analysis, the channels to keep (or exclude), the minimum number of spikes required to analyze a channel, etc.  like for example, which channel to exclude or the time window which is interesting for analyses. In the link below, you can download the blank template file of the guideline **to fill in for your own analyses**. It is called "guideline_void" in the example data folder.

**Note:** Here the example guideline filled in is called "MEA_Spikes_ANA_R_guideline", it's the name by default in `dataset_filtering` function. If you want to save the guideline with an other name, it is possible, and you can put this name in `guideline_file_name` argument of `dataset_filtering` function.

**The guideline has to be saved in the `\data` folder.** This is a view of the example `\data` folder with the guideline filled in and the empty guideline. You can download it in the following link: https://instituteicm-my.sharepoint.com/:f:/g/personal/gaspard_martet_icm-institute_org/Eo0m4uB8wWZBlXsDMaH96bYBepv9jWIjbBIhvK3xMF6GzQ?e=bY4vQS
