...
burst_detection <- function(
  ...
  for (j in 1:nb.condition) {
    ...
    if (nrow(dat) > 0) {
      ...
      isi.ms <- unlist( tapply(dat$within_session_time_ms, list(dat$channel, dat$cluster_id), MEASPIRs:::diff_isi) )
      dat <- data.frame(dat, spike.order, isi.ms)

      if( burst.detection.method == "MI" ) {

        ls.burst <- lapply(unique(dat$channel.clus),
                           function(i) MEASPIRs:::detect_burst_mi(dat$isi.ms[dat$channel.clus == i],
                                                       min_spike_in_burst,
                                                       max_ISI_start_burst,
                                                       max_ISI_end_burst,
                                                       min_IBI,
      ...
      if( burst.detection.method == "chen") {
        ...
        ls.burst <- lapply(unique(dat$channel.clus),
                           function(i) MEASPIRs:::detect_burst_chen(dat$isi.ms[dat$channel.clus == i],
                                                         min_spike_in_burst))
    ...
    if( nrow(dat[ !(is.na(dat$burst)), ]) > 0 ) { # at least one burst found across analyzed electrodes
      ...
      if (sum(as.numeric(table(paste0(burst.start.time$channel, burst.start.time$cluster_id)) > 1))) {
        ...
        ibi.ms <- with(burst.start.time, unlist( tapply(burst.start, list(channel, cluster_id), MEASPIRs:::diff_isi)))
