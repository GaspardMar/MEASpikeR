...
#'
#' @param object Object of class MEASPIRs generated by the \code{dataset_filtering} function
...
#'
#' @examples \dontrun{
#' out.MEA <- MEASPIRs::dataset_filtering(data.path = "C:/Users/prenom.nom/Documents/data",
#' save.rDATA = TRUE)
#'
#' # if you want to work with the MEASPIRs object
...
spikeraster_heatmap <- function(
  ...
  if ( all(!(is.na(object))) & class(object) == "MEASPIRs") {
  ...
  mypal <- MEASPIRs:::select_palette(mypal)
  ...
  for (j in 1:nb.condition) {
    ...
    if ( info.cond$empty.file[j] ) {
      ...
      p1 <- MEASPIRs:::draw_void_rasterplot(shortname, start_TW, end_TW)
      p2 <- MEASPIRs:::draw_void_heatmap(lab.exp, mypal, col.cell.border)
      ...
      if (nrow(dat) > 0 & add.burst == TRUE) { # if_add.burst
        ...
        isi.ms <- unlist( tapply(dat$within_session_time_ms, list(dat$channel), MEASPIRs:::diff_isi) )
        ...
        if( burst.detection.method == "MI" ) {
          ...
          ls.burst <- lapply(unique(dat$channel),
                             function(i) {MEASPIRs:::detect_burst_mi(dat$isi.ms[dat$channel == i],
                                                         min_spike_in_burst,
                                                         max_ISI_start_burst,
                                                         max_ISI_end_burst,
                                                         min_IBI,
        ...
        if( burst.detection.method == "chen") {
          ...
          ls.burst <- lapply(unique(dat$channel),
                             function(i) MEASPIRs:::detect_burst_chen(dat$isi.ms[dat$channel == i], min_spike_in_burst))
