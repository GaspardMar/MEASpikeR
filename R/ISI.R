...
#' @param working_directory Character specifying the working directory where we want to save the output folder
  ...
  for (j in 1:nb.condition) {
    ...
    if ( info.cond$empty.file[j] ) { # If empty file output void plots
      ...
      for (i in 1:nb.electrode) {
        ...
        plot_list[[nc]] <- MEASPIRs:::draw_void_plot_2(i, interval.max, type = 1)
    ...
    if (nrow(dat) > 0) { # if_not_empty_dataset
      ...
      for (i in 1:nb.electrode) {
        ...
        if ( !(i %in% an.ch) ) {
          ...
          plot_list[[nc]] <- MEASPIRs:::draw_void_plot_2(i, interval.max, type = 2)
          ...
          if ( sum(dat$channel == i) == 0 ) { # no spike in channel
            ...
            if (nrow(tmp) == 0) {
              ...
              p <- MEASPIRs:::draw_void_plot_2(i, 300, type = 1)
